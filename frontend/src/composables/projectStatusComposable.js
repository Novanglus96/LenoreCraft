...
async function getProjectStatusesFunction() {
  try {
    const response = await apiClient.get("/project/project_status/list");
    return response.data;
  } catch (error) {
    handleApiError(error, "ProjectStatuses not fetched: ");
  }
}

export function useProjectStatuses() {
  const queryClient = useQueryClient();
  const { data: project_statuses, isLoading } = useQuery({
    queryKey: ["project_statuses"],
    queryFn: () => getProjectStatusesFunction(),
